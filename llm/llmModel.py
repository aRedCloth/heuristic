...
from server.llm.xunfei.chat_spark import get_spark_completion
    ...
    if task == "label_and_explain":#当前文本情感分析
        ...
        if all_text_in_a_string == '':
            ...
            prompt="I have a label list (each element in this list stands for a label category.) and sentences. Now i want you to do a word labeling task following steps below:\
                    ...
                    1. Please read these sentences by words.\
        ...
        func_config={'func_name': 'token_match', 'aspect': 'food', 'pronoun_replace': 'False', 'other_name': [], 'sentiment_labels': ['positive', 'negative','neutral'], 'rules': {}, 
        ...
        prompt+="I am trying to configure it to contruct a Aspecet-based sentiment analysis function finding the sentiment of the aspect(namely the 'aspect' attribute in this template) in a sentence.\
                You can select one a labeling method from 'token-match','structure-match','window-analysis-match'and 'full-text-match' for 'label_method' attribute in the template\
                1. 'token-match' method means firstly locate the position of 'aspect' in a sentence and find the nearest sentiment word(happy or sad for instance) to the aspect. \
                ...
                2.'structure-match' tries to extract a subject-verb-object structure from a sentence and analyze the relationship among these 3 elements , then get a final sentiment.\
                3.'window-analysis-match'  firstly locates the position of 'aspect' and take a parameter 'window_size' as radius, \
                    then it takes the sentiment of this certain range('window_size' words before aspect and 'window_size' behind aspect) as the sentiment of this aspect.\
